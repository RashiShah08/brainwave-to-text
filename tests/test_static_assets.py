...
def test_cortex_mesh_header_matches_its_payload() -> None:
    ...
    assert blob[:4] == b"CTX3", "cortex.bin is missing its magic"

    n_verts, n_tris = struct.unpack_from("<II", blob, 4)
    assert n_verts > 0 and n_tris > 0

    # header + positions + normals + depth + occlusion + region (pad) + indices
    expected = 12 + n_verts * 12 + n_verts * 12 + n_verts * 4 + n_verts * 4
...
def test_cortex_regions_are_within_range() -> None:
    ...
    start = 12 + n_verts * 12 + n_verts * 12 + n_verts * 4 + n_verts * 4
